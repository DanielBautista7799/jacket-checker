...
type SanitizedMetadata = Record<
string,
Exclude<SanitizedMetadataValue, null>
>;
...
const blockedKey =
/(?:email|token|secret|password|authorization|cookie|coordinate|latitude|longitude|\blat\b|\blon\b|image|path|url|prompt|response|vector|address|city|location_name|query_text)/i;
...
value: SanitizedMetadataValue,
): value is PrimitiveMetadataValue {
return (
  typeof value === "string" ||
  typeof value === "number" ||
...
if (Array.isArray(value)) {
  return value
    .slice(0, 8)
    .map(sanitizeValue)
    .filter(isPrimitiveMetadataValue);
}
...
for (const [key, entry] of Object.entries(source).slice(0, 12)) {
  if (!safeKey.test(key) || blockedKey.test(key)) {
    continue;
  }

  const sanitizedValue = sanitizeValue(entry);
...
if (
  ...
  throw new SafeHttpError(
    400,
    "invalid_event",
    "Each analytics event must be an object.",
  );
}
...
if (!allowedEvents.has(eventName)) {
  throw new SafeHttpError(
    400,
    "unsupported_event",
    "An analytics event is not supported.",
  );
}
...
if (!safeModes.has(mode)) {
  throw new SafeHttpError(
    400,
    "unsupported_mode",
    "The analytics experience mode is invalid.",
  );
}

return {
  event_name: eventName,
  route: safeRoute.test(route)
    ? route.slice(0, 120)
    : "/",
  ...
  duration_ms:
    Number.isFinite(duration) && duration >= 0
      ? Math.min(Math.round(duration), 300_000)
      : null,
