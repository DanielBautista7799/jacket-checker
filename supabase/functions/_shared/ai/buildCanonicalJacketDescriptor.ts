...
function ratingLabel(value: unknown, labels: string[]): string {
  const number = Math.min(5, Math.max(1, Math.round(Number(value) || 1)));
  return labels[number - 1];
}

export function buildCanonicalJacketDescriptor(itemValue: unknown): string {
  const item = itemValue && typeof itemValue === "object" && !Array.isArray(itemValue)
    ? itemValue as RawObject
    : {};

  const name = clean(item.name, "unnamed jacket");
  const subtype = clean(item.subtype ?? item.type, "jacket");
  const primaryColor = clean(item.primary_color ?? item.primaryColor ?? item.color, "unknown color");
  const secondaryColor = clean(item.secondary_color ?? item.secondaryColor, "");
  ...
  const warmth = ratingLabel(item.warmth_rating ?? item.warmthRating, [
    "very light warmth",
    "light warmth",
    "medium warmth",
    "high warmth",
  ...
  const formality = ratingLabel(item.formality_rating ?? item.formalityRating, [
    "very casual",
    "casual",
    "versatile",
    "dressy",
