  ...
  const handleAuth = async (event) => {
    ...
    try {
      ...
      if (mode === "sign-up") {
        ...
        const result =
          await signUpWithServerPasswordPolicy({
            email: email.trim(),
            password,
            emailRedirectTo,
          });
        ...
        setAuthMessage(
          result.confirmationRequired
            ? "Account created. Check your email to confirm the account, then return to Jacket Checker."
    ...
    <div>
        ...
        <div className="mb-6 grid grid-cols-2 gap-1 rounded-2xl border border-slate-400/12 bg-black/15 p-1">
          <button
            ...
            className={`flex min-h-11 items-center justify-center gap-2 rounded-xl px-3 text-sm font-extrabold transition ${
              mode === "sign-in"
                ? "bg-blue-400/14 text-sky-100 shadow-[inset_0_0_0_1px_rgba(96,165,250,.18)]"
          ...
          <button
            ...
            className={`flex min-h-11 items-center justify-center gap-2 rounded-xl px-3 text-sm font-extrabold transition ${
              mode === "sign-up"
                ? "bg-violet-400/14 text-violet-100 shadow-[inset_0_0_0_1px_rgba(167,139,250,.18)]"
                : "text-slate-400 hover:bg-white/[0.05] hover:text-white"
            }`}
          >
            <UserPlus
      ...
      {isRecovery && (
        <div className="mb-6">
          <button
            ...
            className="inline-flex items-center gap-2 text-sm font-extrabold text-slate-400 transition hover:text-white"
          >
            <ArrowLeft
              size={16}
              aria-hidden="true"
      ...
      {mode === "sign-in" && (
        <div className="mt-5 grid gap-2 border-t border-slate-400/10 pt-5 sm:grid-cols-2">
          <button
            ...
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl text-sm font-extrabold text-cyan-300 transition hover:bg-cyan-400/[0.06] hover:text-cyan-100"
          >
            <KeyRound
              size={16}
              aria-hidden="true"
          ...
          <button
            ...
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl text-sm font-extrabold text-slate-400 transition hover:bg-white/[0.04] hover:text-white"
          >
            <HelpCircle
              size={16}
              aria-hidden="true"
      ...
      {isRecovery && (
        <p className="mt-5 text-center text-xs leading-5 text-slate-500">
          Recovery responses stay intentionally
          generic so nobody can use this form
          to discover registered accounts.{" "}
          <Link
            to="/"
