  ...
  const signedInLinks = (
    <>
      ...
      <button
        ...
        disabled={signingOut}
        className="min-h-11 whitespace-nowrap rounded-xl px-3 py-2 text-sm font-bold text-slate-300 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-400/30 disabled:opacity-50"
      >
    ...
    <header className="relative z-20 mb-4 rounded-3xl border border-white/10 bg-slate-950/70 p-3 shadow-xl backdrop-blur-xl sm:mb-6 sm:p-4">
      <div className="flex items-center justify-between gap-3">
        <Link to={user ? "/app" : "/"} className="flex min-w-0 items-center gap-3 rounded-2xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-400/30" aria-label="JacketCheck home">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-sky-500 text-white shadow-lg shadow-sky-500/20"><CloudSun size={23} aria-hidden="true" /></span>
