      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <FieldShell icon={Thermometer} label="Cold Tolerance">
          <select
            ...
            onChange={(event) => updateField("cold_tolerance", event.target.value)}
            className={inputClass}
          >
        ...
        <FieldShell icon={Ruler} label="Height (inches)">
          <input
            ...
            onChange={(event) => updateField("height_inches", event.target.value)}
            className={inputClass}
          />
        </FieldShell>
        ...
        <FieldShell icon={CloudRain} label="Rain Sensitivity">
          <select
            ...
            onChange={(event) => updateField("rain_sensitivity", event.target.value)}
            className={inputClass}
          >
        ...
        <FieldShell icon={Waves} label="Wind Sensitivity">
          <select
            ...
            onChange={(event) => updateField("wind_sensitivity", event.target.value)}
            className={inputClass}
          >
        ...
        <div className="md:col-span-2">
          <FieldShell icon={Clock} label="Usual Time Outside">
            <select
              ...
              onChange={(event) => updateField("default_exposure", event.target.value)}
              className={inputClass}
            >
              <option value="short">Short — quick walks only</option>
              <option value="medium">Medium — campus, errands, or a normal outing</option>
        ...
        <div className="md:col-span-2 rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5">
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-slate-300">
              Used only to keep the short jacket styling excerpt aligned with your taste.
            </p>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <FieldShell icon={Shirt} label="Favorite Shoes">
              <select
                ...
                onChange={(event) => updateField("favorite_shoes", event.target.value)}
                className={inputClass}
              >
            ...
            <FieldShell icon={Shirt} label="Default Bottoms">
              <select
                ...
                onChange={(event) => updateField("default_bottoms", event.target.value)}
                className={inputClass}
              >
            ...
            <FieldShell icon={Sparkles} label="Style Influence">
              <select
                ...
                onChange={(event) => updateField("style_influence", event.target.value)}
                className={inputClass}
              >
                <option value="american_streetwear">American streetwear</option>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
                Trend influence slightly updates the style wording while keeping your selected style and weather needs in control.
              </p>
            </div>
            ...
            <label className="flex cursor-pointer items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3 text-sm font-bold text-white">
              <input
                type="checkbox"
                checked={form.use_style_trends}
                onChange={(event) => updateField("use_style_trends", event.target.checked)}
                className="h-4 w-4 accent-violet-400"
              />
              Use current style trends
            </label>
          </div>

          {form.use_style_trends && (
            <div className="mt-5 max-w-md">
              <FieldShell icon={TrendingUp} label="Trend Influence">
                <select
                  value={form.trend_influence}
                  onChange={(event) => updateField("trend_influence", event.target.value)}
                  className={inputClass}
                >
                  <option value="subtle">Subtle — mostly classic</option>
                  <option value="balanced">Balanced — more noticeable</option>
                </select>
              </FieldShell>
            </div>
          )}
